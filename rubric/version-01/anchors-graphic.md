# Anchors Graphic — Pillars & Elevation Paths · v01 DRAFT
...
- **`graphics/judge-anchors.png`** — the anchors: four pillars + elevation paths + the 0–5 scale.
...
purple `#7a4fc0`; pillar colors (print-dark variants): Pace `#4f7d2c`, Quality
`#1f8a7d`, Value `#a8811c`, Happiness `#2f6fb8` (Partnership `#c04f7c` retired
with the pillar in the NorthStar update).
...
## Board 1 — Anchors: "Four pillars. One elevation path each."

Subtitle: *Teams declare 1–2 pillars at registration. Judging starts at the pain:
is it real, is it mapped to the right pillar, and did that pillar move?*

Five color-coded columns, each with three blocks:

| Pillar | The pain looks like | The elevation path | Proof that counts |
|---|---|---|---|
...
3. **~7:30 PM · scoring lap — Score the floor card.** ~10 min per table, five checks
   ...
   to the right pillar? Only then the details. No table-talk between teams.
...
Footer: four pillar chips + qualifying bar (genuinely agentic · ran on the real
project · pillar declared).
